...
fn loads_example_skill_metadata() {
    let skills_dir = format!("{}/../examples/skills", env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(skills
        .iter()
        .any(|s| s.id == "code-review" || s.id == "explaining-code" || s.id == "skill-creator"));
}
...
{}"#,
    ...
    assert!(loaded
        .instructions
        .contains("This is the full instruction body"));
}
