use openskills_runtime::{ExecutionOptions, OpenSkillRuntime, RuntimeError};
use serde_json::json;
...
fn test_wasm_execution_basic() {
    ...
    let wasm_skill = skills
        .iter()
        .find(|s| s.id == "test-wasm-skill" || s.id == "skill-creator");
    if wasm_skill.is_none() {
        panic!(
            "No WASM skill found in examples/skills (expected test-wasm-skill or skill-creator)"
        );
    }
