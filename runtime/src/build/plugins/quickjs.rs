//! QuickJS build plugin using javy CLI + wasm-tools component conversion.
...
pub struct QuickJsBuildPlugin;

impl QuickJsBuildPlugin {
    pub fn new() -> Self {
        Self
    }
    ...
    fn temp_core_wasm_path(&self, output_wasm: &Path) -> PathBuf {
        ...
        let stem = output_wasm
            ...
            .and_then(|s| s.to_str())
            .unwrap_or("skill");
        path.push(format!("{}_core_{}_{}.wasm", stem, pid, ts));
        path
    }
...
impl BuildPlugin for QuickJsBuildPlugin {
    fn name(&self) -> &str {
        "quickjs"
    }
    ...
    fn is_available(&self) -> Result<bool, OpenSkillError> {
        ...
        let javy_ok = check_tool("javy");
        let wasm_tools_ok = check_tool("wasm-tools");
        Ok(javy_ok && wasm_tools_ok)
    }
    ...
    fn compile(
        ...
        if config.verbose {
            eprintln!(
                "QuickJS: compiling {} to core module {}",
                source_file.display(),
                core_wasm.display()
            );
        }

        // Step 1: Compile JS to core WASM module using javy CLI
        let mut javy_cmd = Command::new(javy_path);
        javy_cmd
            .arg("build")
            .arg(source_file)
            .arg("-o")
            .arg(&core_wasm);
        self.run_command("javy build", javy_cmd, config.verbose)?;

        if config.verbose {
            eprintln!(
                "QuickJS: converting core module to WASI 0.3 component at {}",
                output_wasm.display()
            );
        }
        ...
        if config.verbose {
            eprintln!("QuickJS: build complete: {}", output_wasm.display());
        }
    ...
    fn requirements(&self) -> Vec<String> {
        generate_requirements(&["javy", "wasm-tools"], true)
    }
}
