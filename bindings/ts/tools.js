...
function createSkillTools(runtime, options = {}) {
  ...
  // Directories to exclude from recursive file listings by default
  // These are typically large and not useful to list individually
  ...
  // Helper function to truncate output to prevent context overflow
  // Default max is 32KB per field
  function truncateOutput(text, maxBytes = 32768) {
    if (!text || text.length <= maxBytes) {
      return text;
    }
    const halfMax = Math.floor(maxBytes / 2) - 50;
    const head = text.slice(0, halfMax);
    const tail = text.slice(-halfMax);
    const omittedBytes = text.length - halfMax * 2;
    return `${head}\n\n... [${formatBytes(omittedBytes)} truncated] ...\n\n${tail}`;
  }
  ...
  return {
      ...
      execute: async ({ skill_id, script_path, args, input, timeout_ms }) => {
        try {
          ...
          const result = runtime.runSkillTarget(skill_id, {
            ...
            workspaceDir: workspaceDir,  // Pass workspace to ensure consistent output location
          });
          return JSON.stringify(
            {
    ...
    list_workspace_files: tool({
      description: 'List files in the workspace directory. Use this to discover files generated by skills. Automatically excludes node_modules, .pnpm-store, and other large directories.',
      parameters: z.object({
      ...
      execute: async ({ subdir, recursive, pattern }) => {
        try {
          ...
          const walkDir = (dir, basePath = '') => {
            ...
            for (const entry of entries) {
              ...
              if (entry.isDirectory()) {
                // Always skip excluded directories (node_modules, .pnpm-store, etc)
          ...
          if (truncated) {
            ...
            result.message = `Result limited to ${limit} files. Use subdir or pattern to narrow results.`;
          }
          if (excludedDirs.length > 0) {
            result.excludedDirectories = excludedDirs;
          }
